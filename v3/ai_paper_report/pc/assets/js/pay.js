        ...
        for(var i=0; i<increment_goods_arr.length; i++){
            for(var j=0; j<$(".appreciationLI").length; j++){
                if($($(".appreciationLI")[j]).attr('data-janename') == increment_goods_arr[i].short_name || $($(".appreciationLI")[j]).attr('data-janename1') == increment_goods_arr[i].short_name){   //增值服务块显示
                    if(['szsjzz' , 'qkszsjzz'].includes(increment_goods_arr[i].short_name) && getQueryVariable('szsj_check')) {
                        if(getQueryVariable('szsj_check') == 1) {
                            $($(".appreciationLI")[j]).addClass('selected')
                        }else {
                            $($(".appreciationLI")[j]).hide()
                            continue
                        }
                        $($(".appreciationLI")[j]).addClass('prohibited_modify')
                    }else {
                    ...
                    $($(".appreciationLI")[j]).find('.good_name').text(increment_goods_arr[i].name.replace('-极速版', ''))
...
$(".appreciationUL").on('click','.appreciationLI',function(){
    if(!contentTypeIs) return
    if($(this).hasClass('prohibited_modify')) return
...
// 检测实证数据在父元素的可显示子元素中排第几位, 单数左边,双数右边,控制浮窗
function tips_wd_is() {
    const $target = $('.appreciationLI.szsjzz');
    if(!$target) return;
    const $parent = $target.parent();
    const $visibleChildren = $parent.children('.appreciationLI:visible');
    let szsj_index = $visibleChildren.index($target) + 1;
    if(szsj_index % 2 !== 0) {
        $target.addClass('left_wd')
    }else {
        $target.removeClass('left_wd')
    }
}