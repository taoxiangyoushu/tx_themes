...
function scrollTop(this_) {
    // if($(".indexApp").length > 0){
    //     if(this_.scrollTop()>$('#benchmarking').offset().top) {
    //         $('.detailsBox').addClass('fixedClass')
    //     }else {
    //         $('.detailsBox').removeClass('fixedClass')
    //     }
    // }
    ...
    $.ajax({
        ...
        success: function success(result) {
            if(result.code == '200') {
                ...
                if($(".indexApp").length > 0){
                    setPriceInfo(data.project[0].goods_info)
                }
